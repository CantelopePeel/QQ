...
# def cnot_gate(q, control_modes: Tuple[int, int], target_modes: Tuple[int, int],
#               anc_modes=List[Tuple[int, int]]):
#     c0, c1 = control_modes
#     t0, t1 = target_modes
#     anc_modes0, anc_modes1 = anc_modes
#
#     BSgate(theta=pi/4, phi=0)   | (q[t0], q[t1])
#     BSgate(theta=pi/4, phi=0)   | (q[t0], q[c1])
#
#     ns_gate(q, c1, anc_modes0)
#     ns_gate(q, t0, anc_modes1)
#
#     BSgate(theta=pi/4, phi=0)   | (q[t0], q[c1])
#     BSgate(theta=pi/4, phi=0)   | (q[t0], q[t1])
...
def prep_cnot(q, control, target, control_modes, target_modes, anc_modes):
    if control == 0:
        ...
        Vacuum() | q[control_modes[1]]
        # MeasureFock(select=0) | q[control_modes[1]]
    else:
        Vacuum() | q[control_modes[0]]
        Fock(1) | q[control_modes[1]]

    if target == 0:
        Fock(1) | q[target_modes[0]]
        Vacuum() | q[target_modes[1]]
        # MeasureFock(select=0) | q[control_modes[1]]
    else:
        Vacuum() | q[target_modes[0]]
        Fock(1) | q[target_modes[1]]

    # anc_modes0, anc_modes1 = anc_modes
    # Fock(1) | q[anc_modes0[0]]
    # Vacuum() | q[anc_modes0[1]]
...
with prog.context as q:
    Fock(0) | q[0]
    prep_cnot(q, 0, 0, control, target, ancs)
    cnot_gate(q, control, target, ancs)
...
for i in range(runs):
    ...
    print(i, "SAMP SUM: ", samp_avg, result.samples)
...
print("PROB M0:", probs_mode0)

print(acc_prob_modes[:dims] / runs)

for i in range(4):
    print("Mode {}:\n".format(i), np.sum(acc_prob_modes, axis=tuple(m for m in range(4) if m != i)))
